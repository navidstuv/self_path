...
def get_loaders(config):
    ...
    src_loader = DataLoader(lab_train_generator, batch_size=config.src_batch_size, shuffle=True, num_workers=12,
                               pin_memory=False)
    tar_loader = DataLoader(unlab_train_generator, batch_size=config.tar_batch_size , shuffle=True,
                                 num_workers=12, pin_memory=False)
    val_loader = DataLoader(valid_generator, batch_size=config.eval_batch_size, shuffle=False, num_workers=12, pin_memory=False)
    test_loader = DataLoader(test_generator, batch_size=config.test_batch_size, shuffle=False, num_workers=12, pin_memory=False)
